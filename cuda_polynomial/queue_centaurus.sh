...
# # PCI-E latency
# sbatch --partition=GPU --chdir=`pwd` --time=04:00:00 --ntasks=1 --cpus-per-task=16 --gpus-per-task=1 --job-name=pci_l --mem=10G ./bench.sh 1 1 1 1 1 pci_l 10000
...
sbatch --partition=GPU --chdir=`pwd` --time=04:00:00 --ntasks=1 --cpus-per-task=16 --gpus-per-task=1 --job-name=bw_2 --mem=32G ./bench.sh 5120000 2000000000 10000000 5 5 bw_2 1
